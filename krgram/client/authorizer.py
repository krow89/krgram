...
from krgram.client.errors import MTProtocolError, SecurityError
...
from krgram.tl import auth
from krgram.tl.auth import *
...
class AuthKey:
	...
	def _calculate_id(self):
		if self.key is not None:
			self.key_id = Hash.sha1(self.key)[-8:]
		else:
...
class Authorizer:
	...
	def run(self):
		...
		obj = auth.req_DH_params(nonce=nonce,
								 p=p,
								 q=q,
								 server_nonce=server_nonce,
		...
		if register_class is None or register_class.ID != auth.server_DH_inner_data.ID:
			raise UnexpectedResponseError("Unexpected response type from server")
		tlstream = TLBytesStream(answer)
		#tlstream.write(answer[4:])
		resp_tl_obj = auth.server_DH_inner_data().deserialize_from(tlstream)
		g_a = resp_tl_obj.g_a
		...
		retry_id = 0
		data = auth.client_DH_inner_data(nonce=nonce,
										 server_nonce=server_nonce,
										 retry_id=retry_id,
										 g_b=g_b).serialize()
		enc_data = TLEncryptor.encrypt_client_dh(data, server_dh.aes_key_iv)
		obj = auth.set_client_DH_params(nonce=nonce,
										server_nonce=server_nonce,
										encrypted_data=enc_data)
		resp_tl_obj = self._send_plain_req(conn, obj)
		#resp_tl_obj = msg.get_content()
		if resp_tl_obj.ID != auth.dh_gen_ok.ID:
